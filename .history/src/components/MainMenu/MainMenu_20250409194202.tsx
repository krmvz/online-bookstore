  ...
  return (
    <nav className="main_menu">
      ...
      <Link to="/cart" className="cart_link">
        <AiOutlineShoppingCart /> Куржун
        {cartItems.length > 0 && <span className="cart_count">{cartItems.length}</span>}
      </Link>
      {user ? (
        <>
          <Link to="/profile"><FaUserCircle /> Профиль</Link>
          <button onClick={handleLogout} className="auth_btn"><FiLogOut /> Чыгуу</button>
        </>
      ) : (
        <>
          <Link to="/login" className="auth_btn"><FiLogIn /> Кирүү</Link>
