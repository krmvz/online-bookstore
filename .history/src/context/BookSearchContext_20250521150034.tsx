  ...
  const searchBooks = async (searchTerm: string, filters?: SearchFilters) => {
    ...
    try {
      ...
      const transformedBooks = filteredBooks.map(book => {
        // Check if book already has stores property
        if (book.stores) {
          return book;
        }
        
        // Check if book has availability property
        if (!book.availability) {
          // If neither stores nor availability exists, return book with empty stores array
          return {
            ...book,
            stores: []
          };
        ...
        // Map availability to stores if it exists
        return {
          ...book,
