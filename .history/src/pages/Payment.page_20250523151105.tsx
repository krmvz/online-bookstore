...
export function PaymentPage() {
  ...
  const [deliveryAddress, setDeliveryAddress] = useState({
    ...
    city: 'Бишкек',
    phone: '',
    deliverySpeed: 'standard' as 'express' | 'standard' | 'economy'
  });
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    if (e.target.name === 'receiptImage' && e.target instanceof HTMLInputElement && e.target.files) {
  ...
  const validateForm = () => {
    ...
    if (!paymentVerification.transactionId.trim()) {
      newErrors.transactionId = 'Транзакция ID киргизиңиз';
    }
    if (!paymentVerification.receiptImage) {
      newErrors.receiptImage = 'Төлөм квитанциясын жүктөңүз';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = await processPayment(cartItems, deliveryAddress, paymentVerification);
  ...
  const deliveryFee = calculateDeliveryFee(deliveryAddress.city, deliveryAddress.deliverySpeed);
      ...
      <form onSubmit={handleSubmit} className="delivery-form">
        ...
        <div className="payment-details">
          ...
          <div className="payment-verification">
            <h3>Төлөм тастыктоо</h3>
            <div className="form-group">
              <label htmlFor="transactionId">Транзакция ID:</label>
              <input
                type="text"
                id="transactionId"
                name="transactionId"
                ...
                placeholder="Транзакция ID киргизиңиз"
              />
              {errors.transactionId && <span className="error-message">{errors.transactionId}</span>}
            </div>

            <div className="form-group">
              <label htmlFor="receiptImage">Төлөм квитанциясы:</label>
              <input
                type="file"
                id="receiptImage"
                name="receiptImage"
                ...
                className={errors.receiptImage ? 'error' : ''}
              />
              {errors.receiptImage && <span className="error-message">{errors.receiptImage}</span>}
              {paymentVerification.receiptImage && (
                <p className="file-name">{paymentVerification.receiptImage.name}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="form-group">
          <label htmlFor="deliverySpeed">Жеткирүү ылдамдыгы:</label>
          <select
            id="deliverySpeed"
            name="deliverySpeed"
            value={deliveryAddress.deliverySpeed}
            onChange={handleInputChange}
          >
            <option value="express">Тез жеткирүү (1 күн)</option>
            <option value="standard">Стандарттык (3 күн)</option>
            <option value="economy">Үнөмдүү (5 күн)</option>
          </select>
        </div>
