  ...
  const handleAddToCart = async (book: Book) => {
    ...
    try {
      await addToCart({
        bookId: book.id,
        title: book.title,
        price: book.availability && book.availability.length > 0 ? book.availability[0].price : 0,
        quantity: 1,
        ...
        {currentBooks.map((book) => (
          <div key={book.id} className="book_card">
            <Link to={`/books/${book.id}`}>
              <img className="book_image" src={book.imageUrl} alt={book.title} />
              <h3 className="book_title">{book.title}</h3>
            </Link>
            <p className="book_author">{book.author}</p>
            <p className="book_price">{book.availability && book.availability.length > 0 ? `${book.availability[0].price} сом` : 'N/A'}</p>
            <button 
